...
parser.add_argument('--id-prop-p', metavar='N', default='id_prop.csv', type=str, help="Cif list (csv)")
...
def main():
    ...
    #print(model_args)
    # load data
    dataset = CIFData(args.cifpath, id_prop=args.id_prop_p)
...
def lossfn(outs, targets, ignore, criterion, runtyp, cycle):
    col_losses = [0.] * outs.shape[1]
    cols = np.array(range(outs.shape[1]))
    sum_cols = 0.
    for i in cols:
        col_losses[i] = criterion(outs[:, i], targets[:, i])
    ...
    with open(ofname, "a") as lossfile:
        ...
        lossfile.write("\n")
    if ignore:
        cols = np.delete(cols, ignore)
    for i in cols:
        sum_cols += col_losses[i]
    return sum_cols
...
def validate(val_loader, model, criterion, normalizer, test=False):
    ...
    losses = AverageMeter()
    if model_args.task and {'regression', 'r'}:
    ...
    for i, (input, target, batch_cif_ids) in enumerate(val_loader):
        ...
        loss = lossfn(output, target_var, [], criterion, "validation", 1)
    ...
    if test:
        ...
        with open('test_results.csv', 'w') as f:
            for i in range(len(test_cif_ids) - 1):
